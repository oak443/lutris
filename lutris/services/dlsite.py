...
class DlsiteService(OnlineService):
    ...
    def get_works(self, work_ids: list[str]) -> list[dict]:
        ...
        for i in range(0, len(work_ids), batch_size):
            ...
            request = Request(url, headers={"Accept": "application/json", "Content-Type": "application/json"}, cookies=self.load_cookies())
            try:
    ...
    def get_library(self):
        ...
        games = [
            DlsiteGame.new_from_dlsite_game(work) 
            for work in works 
            if work.get("work_type") in game_work_types
        ]
        return games
